...
- 평면도 탭은 Revit Add-in이 동기화한 기본 Floor Plan view 목록, Floor Plan PNG 배경, 방 polygon 구역을 표시합니다. 2D/3D 전환을 지원하며, 3D는 가구/문/창을 복원하지 않고 방 polygon 기반 바닥, 회색 벽체, 선택 방 강조만 표시하는 건축 매스 뷰입니다. 두 모드 모두 같은 `BIM_PHOTO_ROOM_ID` polygon 데이터를 사용해 방 선택과 사진 타임라인을 안정적으로 연결합니다.
