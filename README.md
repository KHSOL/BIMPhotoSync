...
- Viewer는 Revit Add-in이 동기화한 Sheet PDF의 첫 페이지를 브라우저 canvas로 렌더링하고, 그 위에 별도 저장된 SVG Room polygon overlay를 얹습니다. PDF embed/object toolbar에 의존하지 않으므로 Room 클릭 영역이 도면 위에서 직접 동작합니다. Sheet가 없을 경우 기존 floor plan polygon 표시로 fallback합니다. Room polygon 클릭 시 해당 Room 사진 API와 업로드 화면으로 연결합니다.
