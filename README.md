...
- 사진 업로드 탭에서 프로젝트와 방을 선택하고 사진 파일을 한 장 또는 여러 장 선택한 뒤, 공사면, 공종, 내용을 입력해 업로드합니다. 작업일자는 업로드 시점의 날짜와 시각으로 자동 저장되고, 작성자는 로그인 사용자 이름으로 고정됩니다. 업로드가 완료되면 성공 알림을 표시하고 사진 조회 탭으로 이동해 방 기준 목록에서 방금 업로드한 사진을 확인할 수 있습니다. 공사면은 바닥, 기준벽(출입문), 전면벽, 우측벽, 좌측벽, 벽(기타), 천장, 창호, 문, 배관, 전기, 기타로 구분합니다.
...
- 사진 조회 탭의 선택 사진 상세에는 관리자용 AI 분석 검토 패널이 있습니다. 관리자가 요약, 공종, 공사면, 공정 상태를 수정하면 `PhotoAiAnalysis`에 human review 값이 저장되고 `Photo`의 상태/분류에도 반영되어 이후 방 공정 색상과 보고서 근거가 개선됩니다. 관리자는 사진 상세에서 원본 다운로드와 사진 삭제를 수행할 수 있으며, 삭제된 사진은 soft delete되어 일반 조회와 다운로드 대상에서 제외됩니다. 사진 목록이 길어도 넓은 화면에서는 선택 사진 상세 패널이 화면 안에 고정되어 현재 선택 사진과 검토 폼을 계속 확인할 수 있습니다. 이는 현재 구현의 반복 학습 루프이며, 모델 파라미터를 직접 재학습하는 기능은 아닙니다.
...
- 보고서는 관리자 권한 계정에서만 접근합니다. `POST /reports/generate`, `GET /reports`, `GET /reports/:reportId`, `GET /reports/:reportId/export?format=DOCX`, `DELETE /reports/:reportId` 기반으로 초안 데이터를 생성/조회/삭제하고 Word `.docx`로 다운로드할 수 있습니다. 생성 화면의 `AI 지시문` 입력값은 Gemini 보고서 프롬프트에 포함되며, 별도 채팅 UI는 두지 않습니다. 보고서 생성 필터는 프로젝트, 층, 방, 공사면, 공종, 작업일자, 작성자를 분리해 입력합니다. 보고서 분류 순서는 프로젝트 -> 방(실) -> 공사면 -> 작업일자 -> 공종 -> 작성자(작업자)입니다. 날짜 끝 필터는 해당 날짜의 23:59:59까지 포함하고, `6월 작업보고서`처럼 월 단위 지시가 입력되면 명시 날짜가 없을 때 해당 월 범위를 자동 적용합니다. `trade_category_id`가 선택된 보고서 생성은 legacy `trade`보다 공종 분류 ID를 우선 적용해 사진 수가 0으로 잘못 줄어드는 경우를 막습니다. DOCX 내보내기는 `apps/api/templates/report-template.docx` 제공 양식을 기준으로 기본정보, 날짜별 작업일지, 종합판단, 2페이지 사진 슬롯과 방/공종/공사면/공사일/작업내용을 채웁니다. 이미지 원본을 읽지 못하면 텍스트 근거만 남기고 파일 다운로드는 계속 진행합니다.
...
- 보고서는 DB에 JSON 구조로 저장되며 다운로드 시 `.docx`로 내보낼 수 있습니다. 내보내기 문서는 제목, 생성일, 작성자, 상황분석, 날짜별 작업일지, 종합판단, 비교 사진 근거, 메모를 채우며, 생성 화면의 `AI 지시문` 입력값은 보고서 생성 프롬프트에 반영됩니다. DOCX는 `apps/api/templates/report-template.docx` 원본 워드 양식을 기준으로 1페이지 작업일지와 2페이지 사진대지를 생성하고, 사진 슬롯마다 방/공종/공사면/공사일/작업내용을 함께 저장합니다.
