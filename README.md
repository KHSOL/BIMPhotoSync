...
- 대시보드는 전체 프로젝트 또는 선택한 프로젝트 기준으로 방, 사진, AI 분석, 보고서, Revit 모델 연결 현황과 연결된 프로젝트 수를 API에서 집계해 표시합니다. 사진 분포는 공종 기준으로만 보여 주며, 일반 작업자 계정에는 보고서 관련 카드가 표시되지 않습니다.
...
- 사진 화면은 사진 조회 탭과 사진 업로드 탭을 분리합니다. 조회 탭에는 한 줄형 자동 조회 필터와 사진 상세만 표시하고, 업로드 탭에는 업로드 입력, 내용 입력, 업로드 상태 안내만 표시합니다.
...
- 보고서는 관리자 권한 계정에서만 접근합니다. `POST /reports/generate`, `GET /reports`, `GET /reports/:reportId`, `GET /reports/:reportId/export?format=DOCX` 기반으로 초안 데이터를 생성/조회하고 Word `.docx`로 다운로드할 수 있습니다. 생성 화면의 `AI 지시문` 입력값은 Gemini 보고서 프롬프트에 포함되며, 별도 채팅 UI는 두지 않습니다. 보고서 분류 순서는 프로젝트 -> 방(실) -> 공사면 -> 작업일자 -> 공종 -> 작성자(작업자)입니다. 날짜 끝 필터는 해당 날짜의 23:59:59까지 포함하고, `6월 작업보고서`처럼 월 단위 지시가 입력되면 명시 날짜가 없을 때 해당 월 범위를 자동 적용합니다. `trade_category_id`가 선택된 보고서 생성은 legacy `trade`보다 공종 분류 ID를 우선 적용해 사진 수가 0으로 잘못 줄어드는 경우를 막습니다. DOCX 내보내기는 `apps/api/templates/report-template.docx` 제공 양식을 기준으로 기본정보, 날짜별 작업일지, 종합판단, 2페이지 사진 슬롯과 방/공종/공사면/공사일/작업내용을 채웁니다. 이미지 원본을 읽지 못하면 텍스트 근거만 남기고 파일 다운로드는 계속 진행합니다.
...
apps/mobile
  Expo React Native 현장 앱. 자체 이메일/비밀번호 로그인, 프로젝트 선택/참여, 카메라/갤러리 사진 추가, 방별 사진 조회, 내 정보 확인 흐름을 제공합니다.
