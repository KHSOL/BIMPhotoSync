...
- 보고서는 관리자 권한 계정에서만 접근합니다. `POST /reports/generate`, `GET /reports`, `GET /reports/:reportId`, `GET /reports/:reportId/export?format=` 기반으로 초안 데이터를 생성/조회하고 JSON, `.xlsx`, `.docx`, `.pdf`, `.hwpx`로 다운로드할 수 있습니다. 생성 화면의 `AI 지시문` 입력값은 Gemini 보고서 프롬프트에 포함되며, 별도 채팅 UI는 두지 않습니다. 보고서 분류 순서는 프로젝트 -> 방(실) -> 공사면 -> 작업일자 -> 공종 -> 작성자(작업자)입니다. 날짜 끝 필터는 해당 날짜의 23:59:59까지 포함하고, `6월 작업보고서`처럼 월 단위 지시가 입력되면 명시 날짜가 없을 때 해당 월 범위를 자동 적용합니다. `trade_category_id`가 선택된 보고서 생성은 legacy `trade`보다 공종 분류 ID를 우선 적용해 사진 수가 0으로 잘못 줄어드는 경우를 막습니다. XLSX 내보내기는 `apps/api/templates/report-template.xlsx` 제공 양식을 그대로 복제한 뒤 표지/주간공정보고/사진대지/전경사진 셀을 채우고, 실제 보고서 사진을 사진대지 슬롯에 삽입하며, 사진 메타데이터가 길어도 줄바꿈과 행 높이 확장으로 잘리지 않게 유지합니다. 이미지 원본을 읽지 못하면 텍스트 근거만 남기고 파일 다운로드는 계속 진행합니다.
...
apps/mobile
  Expo React Native 현장 앱. 이메일/비밀번호 로그인 후 상단 프로젝트 선택, 중앙 촬영 버튼, 업로드 초안, 층별 Room 검색 시트를 중심으로 한 작업자 흐름을 제공합니다.
...
- 보고서는 DB에 JSON 구조로 저장되며 다운로드 시 JSON, `.xlsx`, `.docx`, `.pdf`, `.hwpx`로 내보낼 수 있습니다. 내보내기 문서는 제목, 생성일, 작성자, 상황분석, 변화 과정, 비교 사진 근거, 메모를 채우며, 생성 화면의 `AI 지시문` 입력값은 보고서 생성 프롬프트에 반영됩니다. XLSX는 `apps/api/templates/report-template.xlsx` 원본 워크북을 템플릿으로 사용해 기존 시트, 병합 셀, 열 너비, 페이지 설정, 도형/이미지를 보존한 채 보고서 데이터 셀과 사진 슬롯만 채우고, 긴 한국어 근거 문장은 줄바꿈 가능한 스타일과 확대된 행 높이로 유지합니다.
