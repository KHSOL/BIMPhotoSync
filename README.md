...
- Revit 2025 Add-in에서 프로젝트 연결, Room Sync, 기본 Floor Plan view 동기화, Floor Plan/Sheet PDF export, 도면 위 Room polygon overlay 동기화, Room 선택 시 사진 타임라인 조회
...
- 로그인 후 좌측 내비게이션으로 Dashboard, Projects, Rooms, Photos, Floor Plan, Sheets, Reports, Audit, My Page, Admin에 접근합니다.
...
- Floor Plan 탭은 Revit Add-in이 동기화한 기본 Floor Plan view 목록, Floor Plan PDF 배경, Room polygon 구역을 표시합니다. Room 구역을 클릭하면 해당 Room 사진 API와 업로드 화면으로 연결됩니다.
...
`Sync Rooms`는 canonical Room ID 매핑과 `BIM_PHOTO_ROOM_ID` 기록을 담당합니다. 이후 `Sync Floor Plans`가 각 Floor Plan view 안에 실제로 보이는 Room만 다시 수집해 PDF 배경과 polygon zone을 Floor Plan 탭에 올립니다. 사용자는 Floor Plan 위 zone을 클릭해 사진을 올리지만, 사진 데이터는 항상 canonical Room에 저장됩니다. `Sync Floor Plans`는 Revit Project Browser의 기본 Floor Plan 그룹에 해당하는 1차 Level view만 대상으로 삼고, 확대도, life safety, working, wall base/top 같은 파생 view는 제외합니다. 실행 시 기존 Floor Plan 동기화 결과를 먼저 지워 오래된/과다 동기화 도면이 웹에 남지 않게 합니다. `Sync View`는 현재 열린 평면도 또는 현재 Sheet만 동기화하고, `Sync Sheets`는 전체 Sheet PDF와 Sheet overlay를 명시적으로 동기화합니다. Floor Plan/Sheet PDF는 화면 배경이고, Room 클릭은 PDF 내부 텍스트/선분이 아니라 별도 overlay polygon으로 처리합니다. Room boundary는 Revit `SpatialElement.GetBoundarySegments` 기반이며, Floor Plan 좌표는 Revit 2025 API의 `View.GetModelToProjectionTransforms()`를 우선 사용하고, Sheet Viewport 좌표 변환은 `View.GetModelToProjectionTransforms()`와 `Viewport.GetProjectionToSheetTransform()` 조합을 사용합니다. Sheet sync payload는 대형 프로젝트에서도 API body limit에 덜 민감하도록 Add-in이 Sheet 단위로 나누어 업로드합니다. API는 Sheet 저장 transaction timeout을 120초로 설정하고 Room overlay를 batch insert해 Snowdon처럼 overlay가 많은 Sheet도 저장할 수 있게 합니다.
...
revit-addin/package-addin.ps1
  Revit Add-in Release/Debug 산출물을 설치 가능한 ZIP 패키지로 묶습니다.

revit-addin/install.ps1
  패키지 또는 로컬 빌드 산출물을 현재 Windows 사용자의 Revit Addins 폴더에 설치합니다.
...
```powershell
powershell -ExecutionPolicy Bypass -File revit-addin\package-addin.ps1 -Configuration Release
powershell -ExecutionPolicy Bypass -File revit-addin\.package\BimPhotoSyncAddin-Revit2025-Release\install.ps1
```

설치 ZIP은 `revit-addin\dist\BimPhotoSyncAddin-Revit2025-Release.zip`에 생성됩니다. 다른 Windows/Revit 사용자는 ZIP을 풀고 `install.ps1`만 실행하면 `%APPDATA%\Autodesk\Revit\Addins\2025\BimPhotoSync`와 manifest가 자동 구성됩니다. 삭제는 같은 패키지의 `uninstall.ps1`로 처리합니다.
...
6. 현재 열어둔 평면도 또는 Sheet만 웹에 올리려면 `Sync View`를 누릅니다.
7. 프로젝트의 전체 Floor Plan view를 Floor Plan 탭에 올리려면 `Sync Floor Plans`를 누릅니다.
8. 전체 Sheet PDF와 Sheet별 Room overlay를 Sheets 탭에 올려야 할 때만 `Sync Sheets`를 누릅니다. 큰 Revit 모델에서는 시간이 오래 걸릴 수 있으므로 먼저 `Sync View` 또는 `Sync Floor Plans`로 검증하는 흐름을 권장합니다.
...
- Sync Floor Plans가 전체 Floor Plan view PDF와 Room polygon을 백엔드에 업로드합니다.
- Sync Sheets가 전체 Sheet PDF와 Room overlay polygon을 백엔드에 업로드합니다.
- Floor Plan 탭에서 Floor Plan Room polygon을 클릭할 수 있습니다.
