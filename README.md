...
- Revit 2025 Add-in에서 프로젝트 연결, Room Sync, 기본 Floor Plan view 동기화, Floor Plan PNG export, Sheet PDF export, 기존 Revit 3D view 기반 Floor Plan별 3D section export, 도면 위 Room polygon overlay 동기화, Room 선택 시 사진 타임라인 조회
- Next.js 웹 콘솔, Expo 모바일 앱, NestJS API, BullMQ worker, Prisma/PostgreSQL 데이터 모델

현재 범위에서 APS Viewer, Dynamo graph 실행, 고급 AI 진단 모델 재학습은 구현 대상이 아닙니다. 다만 Floor Plan/Sheet 구조는 정식 제품 단계에서 APS Viewer로 교체할 수 있도록 도면 표시 레이어와 Room/구역 선택 레이어를 분리합니다. 보고서는 현재 Word(DOCX) 다운로드를 제공합니다.

## 웹 콘솔 동작 범위

- 로그인 후 좌측 내비게이션으로 대시보드, 프로젝트, 방 목록, 사진, 평면도, 시트, 내 정보에 접근합니다. 보고서와 감사는 관리자 권한 계정에서만 표시됩니다.
- 대시보드는 전체 프로젝트 또는 선택한 프로젝트 기준으로 방, 사진, AI 분석, 보고서, Revit 모델 연결 현황과 연결된 프로젝트 수를 API에서 집계해 표시합니다. 이슈 사진 카드는 별도로 표시하지 않고, 사진 분포는 공종 기준으로만 보여 주며, 일반 작업자 계정에는 보고서 관련 카드가 표시되지 않습니다.
...
- 사진 화면은 사진 조회 탭과 사진 업로드 탭을 분리합니다. 조회 탭에는 프로젝트, 층, 방, 공종, 공사면, 기간을 분리한 자동 조회 필터와 사진 상세만 표시하고, 업로드 탭에는 업로드 입력, 내용 입력, 업로드 상태 안내만 표시합니다. 사진 상세의 위치는 Room ID가 아니라 `층 / 방 번호 방 이름` 형태로 표시합니다.
- 사진 업로드 탭에서 프로젝트와 방을 선택하고 사진 파일을 한 장 또는 여러 장 선택한 뒤, 공사면, 공종, 작업일자, 내용을 입력해 업로드합니다. 작업일자는 과거 사진 등록을 위해 사용자가 직접 선택하고, 작성자는 로그인 사용자 이름으로 고정됩니다. 업로드가 완료되면 성공 알림을 표시하고 사진 조회 탭으로 이동해 방 기준 목록에서 방금 업로드한 사진을 확인할 수 있습니다. 공사면은 바닥, 기준벽(출입문), 전면벽, 우측벽, 좌측벽, 벽(기타), 천장, 창호, 문, 배관, 전기, 기타로 구분합니다.
- 사진 조회 탭의 프로젝트, 방, 공종, 공사면, 기간 선택은 변경 즉시 실제 API 조회를 수행합니다. 선택 방 정보와 방 공정 상태는 별도 조회 기준 패널이 아니라 선택 사진 상세 영역에 함께 표시됩니다. 관리자 권한 계정은 방 목록의 공종 선택 영역에서 현장 표시용 공종 분류를 추가/삭제할 수 있습니다. 공종 분류는 DB의 `TradeCategory`로 관리되며, 기본 공종은 프로젝트/회사 기준으로 자동 생성되고 사용자 지정 공종은 사진과 보고서 필터에 `trade_category_id`로 저장됩니다.
- 사진 조회 탭의 선택 사진 상세에는 관리자용 AI 분석 검토 패널이 있습니다. 사진의 업로드/촬영 시각은 초 단위까지 표시합니다. 관리자가 요약, 공종, 공사면, 공정 상태를 수정하면 `PhotoAiAnalysis`에 human review 값이 저장되고 `Photo`의 상태/분류에도 반영되어 이후 방 공정 색상과 보고서 근거가 개선됩니다. 관리자는 사진 상세에서 원본 다운로드와 사진 삭제를 수행할 수 있으며, 삭제된 사진은 soft delete되어 일반 조회와 다운로드 대상에서 제외됩니다. 사진 목록이 길어도 넓은 화면에서는 선택 사진 상세 패널이 화면 안에 고정되어 현재 선택 사진과 검토 폼을 계속 확인할 수 있습니다. 이는 현재 구현의 반복 학습 루프이며, 모델 파라미터를 직접 재학습하는 기능은 아닙니다.
- 방 목록의 공정 진행률은 Room에 연결된 공종별 사진 상태를 기준으로 계산합니다. 선택된 공종에 사진이 없으면 0점(시작 전), 사진이 1장 이상 있으면 0.5점(진행 중), 사진 메모/AI 분석 또는 상태에 `완료`가 있으면 1점(완료)으로 계산합니다. 방 진행률은 `(선택 공종 점수 합계 / 선택 공종 수) x 100%`이며, 방 목록에서 공종 체크를 바꾸면 즉시 다시 계산합니다. `전체 공종` 체크를 해제하면 모든 공종 선택이 해제되어 모든 방 진행률이 0%로 표시되고, 이후 필요한 공종만 하나씩 선택할 수 있습니다.
...
- 보고서는 관리자 권한 계정에서만 접근합니다. `POST /reports/generate`, `GET /reports`, `GET /reports/:reportId`, `GET /reports/:reportId/export?format=DOCX`, `DELETE /reports/:reportId` 기반으로 초안 데이터를 생성/조회/삭제하고 Word `.docx`로 다운로드할 수 있습니다. 생성 화면의 `AI 지시문`과 `AI Model` 선택값은 보고서 생성 프롬프트와 provider 라우팅에 포함되며, Gemini/OpenAI/Claude 중 하나를 선택할 수 있고 별도 채팅 UI는 두지 않습니다. 보고서 생성 필터는 프로젝트, 층, 방, 공사면, 공종, 작업일자, 작성자를 분리해 입력합니다. 보고서 분류 순서는 프로젝트 -> 방(실) -> 공사면 -> 작업일자 -> 공종 -> 작성자(작업자)입니다. 날짜 끝 필터는 해당 날짜의 23:59:59까지 포함하고, `6월 작업보고서`처럼 월 단위 지시가 입력되면 명시 날짜가 없을 때 해당 월 범위를 자동 적용합니다. `trade_category_id`가 선택된 보고서 생성은 legacy `trade`보다 공종 분류 ID를 우선 적용해 사진 수가 0으로 잘못 줄어드는 경우를 막습니다. DOCX 내보내기는 `apps/api/templates/report-template.docx` 제공 양식을 기준으로 기본정보, 작업일지, 종합판단, 사진 슬롯과 방/공종/공사면/공사일/작업내용을 채웁니다. 작업일지는 작업일자, 촬영일시, 업로드일시, 사진 ID 순서로 등록 사진 1장당 1행만 생성하고 사진이 없는 날짜 행은 만들지 않으며, 작업 항목은 `공사면 / 실제 작업명`, 작업 상세내용은 사진과 작업자 입력을 함께 반영한 AI 문장, 비고는 작업자명이 아니라 주의사항/확인사항으로 작성합니다. Word 표에는 `tblGrid`를 포함해 열 폭을 안정화하고 동적 문단/표 글꼴은 Malgun Gothic으로 고정합니다. 이미지 원본을 읽지 못하면 텍스트 근거만 남기고 파일 다운로드는 계속 진행합니다.
- 감사는 관리자 권한 계정에서만 접근합니다. 현재 프로젝트 참여자 목록을 실제 API로 조회하고 사용자를 선택하면 이름, 이메일, 회사, 전역 권한, 프로젝트 권한, 참여일을 확인할 수 있습니다. 관리자는 프로젝트 참여 작업자를 관리자로 승급할 수 있고, 승급된 사용자는 `MANAGER` 역할과 관리자용 사진/감사/보고서 권한을 사용합니다. 사진 생성, 보고서 생성, 공종 관리, 사진 삭제, 권한 변경 같은 활동 로그만 DB의 `AuditEvent`에서 조회하며 로그인 기록은 화면에 표시하지 않습니다. 작업 유형별 활동 그래프는 실제 생성/수정/삭제 로그 비율을 기준으로 표시합니다. 활동 로그 상세 정보는 긴 문장도 여러 줄로 표시합니다.
- 내 정보는 로그인 사용자 이름, 역할, 회사명, 계정 정보를 표시합니다. 프로필 사진은 presigned URL로 Object Storage에 업로드하고 `users.avatar_object_key`에 저장되어 좌측 내비게이션 사용자 원형 아바타에 지속 반영됩니다.
...
로컬 Railway CLI가 이미 프로젝트에 연결되어 있어도 저장된 CLI 세션 토큰이 만료되거나 계정이 바뀌면 `railway whoami`, `railway logs`, proxy 관련 명령에서 브라우저 인증창이 반복해서 뜰 수 있습니다. 배포 검증만 필요한 경우에는 Railway CLI/MCP를 호출하지 않고 GitHub deployment 상태와 `https://api-production-1d018.up.railway.app/api/v1/health` healthcheck로 확인합니다. CLI 자체를 다시 써야 할 때만 `railway logout` 후 현재 Railway 계정으로 다시 로그인합니다.
...
- API는 Helmet 보안 헤더, gzip/deflate 압축, reverse proxy 신뢰 설정, graceful shutdown hook을 켜고 실행합니다. Revit Floor Plan/Sheet 동기화처럼 JSON payload가 큰 요청을 위해 `JSON_BODY_LIMIT`과 `FORM_BODY_LIMIT` 기본값은 `25mb`입니다.
- 운영 CORS는 API 환경변수 `CORS_ORIGINS`에 Vercel 웹 도메인을 쉼표 구분으로 넣어 제한합니다. 로컬 개발에서는 값을 비워 둘 수 있지만, 운영 Railway API에는 예를 들어 `https://<vercel-domain>` 또는 커스텀 도메인을 지정합니다.
...
- 로컬 Node는 22 계열을 사용합니다. 처음 실행하거나 터미널이 Node 24 이상을 잡고 있으면 `nvm use`를 먼저 실행합니다.
- `npm run dev`는 실제 NestJS API를 `http://localhost:4000`에서, Next.js 웹 콘솔을 `http://localhost:3001`에서 함께 실행합니다. 로컬 3000 포트는 다른 API/서비스가 점유할 수 있으므로 웹 개발 서버 기본 포트는 3001로 고정합니다.
- API만 따로 실행해야 하는 경우 `npm run dev:api`, 웹만 따로 실행해야 하는 경우 `npm run dev:web`를 사용합니다.
- 로컬 `.env.local`의 `NEXT_PUBLIC_API_BASE_URL`이 `http://localhost:4000/api/v1`이면 로그인과 데이터 조회는 이 API 서버가 켜져 있어야 동작합니다. API는 `apps/api/.env`의 `DATABASE_URL`과 `DIRECT_URL`에 연결된 PostgreSQL을 실제 DB로 사용합니다.
...
apps/mobile
  Expo React Native 현장 앱. 자체 이메일/비밀번호 로그인, 프로젝트 선택/참여, 카메라/갤러리 사진 추가, 방별 사진 조회, 사진 상세/다운로드, 관리자 사진 삭제, 내 정보 확인 흐름을 제공합니다.
...
| Project Members | `GET /projects/:projectId/members`, `PATCH /projects/:projectId/members/:memberId/promote-manager` |
...
| Photos | `POST /photos`, `GET /photos`, `GET /photos/:photoId`, `GET /photos/:photoId/object`, `DELETE /photos/:photoId` |
...
| Reports | `POST /reports/generate`, `POST /reports/chat`, `GET /reports`, `GET /reports/:reportId`, `GET /reports/:reportId/export?format=DOCX` |
...
| `MANAGER` | 프로젝트 참여자 관리, 사진 검토/다운로드/삭제, 감사/보고서 보드 접근 |
...
- 방 목록의 공정 진행률은 방 자체 입력값이 아니라 해당 방에 연결된 공종별 사진과 AI 분석 결과의 `progress_status` 집계에 기반합니다. 선택된 공종에 사진이 없으면 0점, 사진이 1장 이상 있으면 0.5점, 사진 메모/AI 검토에 완료가 있으면 1점으로 환산해 퍼센트를 계산합니다. 사진 설명에 완료/이슈 키워드가 있으면 worker가 `COMPLETED` 또는 `BLOCKED`로 분류하고, 그 외에는 기본적으로 `IN_PROGRESS` 또는 `PENDING_REVIEW` 흐름을 탑니다. 방 상태 뱃지는 선택된 모든 공종이 완료일 때만 `완료`, 일부 공종에 진행/완료 흔적이 있으면 `진행중`, 모든 선택 공종이 비어 있으면 `시작 전`입니다.
- 보고서는 DB에 JSON 구조로 저장되며 다운로드 시 `.docx`로 내보낼 수 있습니다. 보고서 생성 AI는 `work_log`와 `judgment` 구조를 반환하도록 프롬프트됩니다. `work_log`는 등록 사진 1장당 1행이며 `공사면 / 실제 작업명`, 상세 작업 내용, 비고/주의사항을 분리합니다. `judgment`는 품질 상태, 특이 사항, 종합 의견을 전체 공정 흐름 기준으로 작성합니다. DOCX는 `apps/api/templates/report-template.docx` 원본 워드 양식을 기준으로 1페이지 작업일지와 2페이지 사진대지를 생성하고, 사진 슬롯마다 방/공종/공사면/공사일/작업내용을 함께 저장합니다.
...
| 영역 | 기술 | 선택 이유 |
| --- | --- | --- |
| Web | Next.js 15, React 19, TypeScript | Vercel 배포와 App Router metadata/OG 설정에 적합하고 운영 콘솔 개발 속도가 빠릅니다. |
...
| Reports AI | Gemini/OpenAI/Claude optional | 보고서 초안 생성을 선택한 외부 LLM으로 확장할 수 있고, 미설정 시에도 구조화된 리포트 데이터를 유지합니다. |
...
현재 웹 Floor Plan/Sheets는 export asset + Room overlay 방식입니다. Floor Plan은 Revit PDF page 여백과 view projection 좌표가 어긋나는 문제를 줄이기 위해 View PDF가 아니라 PNG view image를 업로드하고, Sheets는 PDF를 유지합니다. `Sync Floor Plans`와 `3D Model`은 Revit Project Browser의 Floor Plan view를 Level별로 묶은 뒤, 뷰 이름이 Level 이름과 맞고 Room이 실제로 수집되는 1차 후보를 Level마다 1개 선택합니다. 이 방식은 L1처럼 Room은 존재하지만 이전의 엄격한 기본 평면도 필터에서 빠지던 Floor Plan을 포함하면서도 확대도, life safety, working, wall base/top 같은 파생 view가 과다 동기화되는 문제를 피합니다. 3D 전환은 Revit Add-in의 `3D Model` 동기화가 새 isometric `BPS 3D - <Floor Plan>` View3D를 만든 뒤 각 Floor Plan의 crop/Room 경계 기준 section box를 transaction 안에서 갱신하고 OBJ로 업로드하면 Three.js `OBJLoader`로 렌더링합니다. Revit 문서에서 새 3D view를 만들 수 없을 때만 기존 비투시 3D view 복제를 fallback으로 사용합니다. OBJ export는 벽, 바닥, 천장, 지붕, 계단, 난간, 기둥, 구조 프레임 카테고리를 대상으로 하며 가구, 문, 창, 커튼월, 일반 모델은 제외합니다. Add-in은 section box와 교차하는 요소를 찾은 뒤 삼각형 단위로 section box 내부 geometry만 OBJ에 기록하므로, 긴 벽/바닥/구조 부재 전체가 다른 층 3D에 섞여 보이지 않게 합니다. Floor Plan별 3D section box는 먼저 view-scoped Room collector를 사용하고, 그 결과가 비면 같은 Level의 문서 전체 Room bounding box로 다시 계산하며, 그것도 불가능하면 Floor Plan crop box만으로 fallback 합니다. `Sync Floor Plans`도 view-scoped Room collector가 빈 결과를 반환하면 같은 Level의 Room으로 overlay를 다시 계산합니다. 이 fallback은 Room 이름 매칭을 쓰지 않고 Revit Room level/document 정보만 사용하며, canonical Room 연결은 계속 `BIM_PHOTO_ROOM_ID` 기준으로 유지합니다. `%APPDATA%\\BimPhotoSync\\validation.log`에는 각 Floor Plan이 view-scoped bounds, same-level document fallback, crop-box fallback 중 무엇으로 export 되었는지와 생성된 Revit 3D view, section box 좌표가 기록됩니다. API는 같은 Floor Plan `source_view_id`의 3D 자산을 누적하지 않고 최신 자산으로 교체합니다. Floor Plan sync payload는 2D PNG overlay용 view projection `polygon`과 OBJ 3D 선택용 Revit model coordinate `model_polygon`을 함께 저장합니다. 웹 3D 뷰어는 Revit OBJ에 같이 포함될 수 있는 작은 부유 부품과 얇은 상세 객체를 숨기고, 보이는 mesh의 최저점을 기준으로 모델을 바닥 그리드 y=0에 맞춰 정렬합니다. 3D 선택 overlay는 `model_polygon`을 OBJ와 같은 좌표계인 Revit `(X, Z, -Y)` 기준으로 배치해 Room polygon 클릭이 모델 위에서 동작하도록 유지합니다. 3D 모델 자산은 `RevitModelAsset`으로 관리되고 R2/S3 object key, MIME type, file size, checksum, source view id를 저장합니다. 웹과 API는 현재 선택된 Floor Plan과 `source_view_id`가 정확히 일치하는 3D 자산만 사용하며, 전체 `{3D}` 모델은 Floor Plan 3D 후보에서 제외합니다. 매칭되는 OBJ가 없으면 기존 Room polygon 기반 axonometric fallback과 최신 Add-in에서 `3D Model`을 다시 실행하라는 안내를 표시합니다. 새 Add-in 설치 후 기존 프로젝트를 갱신할 때는 Revit에서 `Sync Floor Plans`를 먼저 실행해 `model_polygon`을 저장하고, 이어서 `3D Model`을 실행해 Floor Plan별 3D 자산을 교체 업로드합니다. 정상 동기화 후 Revit 알림은 `Synced N Floor Plan section 3D models.` 형식이어야 하며, 웹 3D 모델 선택에는 `{3D}`가 아니라 `L2`, `L3`처럼 Floor Plan 이름이 표시되어야 합니다.

모바일 앱은 `apps/mobile`의 Expo React Native 앱입니다. monorepo에서 Expo entry가 루트 `App`을 찾지 않도록 Expo Go와 일반 export는 `apps/mobile/index.js`를 진입점으로 사용합니다. Android native release bundling은 Expo `export:embed`가 monorepo 루트를 entry 기준으로 다시 해석할 수 있어 repo root `index.js` shim에서 `apps/mobile/index.js`로 위임하고, `apps/mobile/metro.config.js`는 workspace root를 watch 대상에 포함합니다. 현재 모바일 흐름은 자체 이메일/비밀번호 인증만 지원하며 Google/Apple 소셜 로그인은 표시하지 않습니다. 로그인 화면 상단은 생성 이미지 asset `apps/mobile/assets/login-hero.png`를 사용합니다. 홈에서는 참여 중인 프로젝트 선택과 촬영/사진 추가 진입을 제공하고, 프로젝트 선택은 한 화면에 여러 프로젝트가 보이는 하단 시트로 열리며 선택/이동/탭/뒤로가기 아이콘은 Expo Feather 벡터 아이콘을 사용합니다. 프로젝트 탭에서는 내가 참여한 프로젝트 목록, 접근키 참여, 선택 프로젝트의 인증 헤더 기반 비공개 사진 썸네일/상세 보기와 사진이 있는 방만 표시하는 필터 시트를 제공합니다. 사진 필터는 층을 먼저 고른 뒤 해당 층의 방만 고르는 2단계 흐름이며, 사진 상세는 큰 이미지, 다운로드, 작업 내용, AI 요약, 올린 사람, 올린 시간, 촬영 시간을 초 단위까지 함께 표시합니다. 업로드 화면에서는 작성자 입력란을 표시하지 않고 로그인 사용자 이름을 저장하며, 작업일자는 `YYYY-MM-DD` 형식으로 직접 입력해 과거 사진도 등록할 수 있습니다. 사진 선택은 `사진 추가` 버튼으로 제공하며, 버튼을 누르면 카메라 촬영 또는 갤러리 선택 중 하나를 고를 수 있고 여러 장을 한 번에 업로드할 수 있습니다. 내 정보 탭은 웹의 내 정보 화면과 같은 이름, 이메일, 역할, 회사, 참여 프로젝트 수, Room 데이터 수를 보여 줍니다. Android native 프로젝트는 `apps/mobile/android`에 고정되어 있어 `npm --workspace apps/mobile run android`로 로컬 Android 실행을 확인할 수 있습니다. 현장 개발 테스트는 같은 Wi-Fi에서 `npm --workspace apps/mobile run start:lan`을 실행한 뒤 iPhone/Galaxy의 Expo Go로 QR을 스캔하는 방식이 가장 안정적입니다. 외부망 테스트가 필요하면 `npm --workspace apps/mobile run start:tunnel`을 사용할 수 있지만, ngrok 터널은 네트워크/방화벽 상태에 따라 시간 초과될 수 있습니다. 터널이 연결된 동안에는 멀리 있는 사람도 Expo Go에서 `exp.direct` URL 또는 QR을 열어 앱을 사용할 수 있지만, 개발 PC와 터미널이 켜져 있어야 하고 Metro 서버/터널이 끊기면 즉시 접속할 수 없습니다. 실제 배포판처럼 안정적으로 여러 사람이 쓰려면 Android APK/EAS build 또는 iOS TestFlight 배포를 사용합니다. 흰 화면이나 오래된 번들 오류가 보이면 `cd apps/mobile && npx expo start --tunnel --clear`로 Metro 캐시를 비워 재시작합니다. Expo SDK 54와 React Native 0.81은 React 19 계열을 기준으로 동작하므로 루트, 웹, 모바일 워크스페이스의 React/React DOM 버전을 19.1.0으로 정렬합니다. 루트에 React 18이 설치되면 Android APK에서 Hermes 런타임이 `Cannot read property 'S' of undefined` 오류로 종료될 수 있습니다. 터널 실행에 필요한 `@expo/ngrok`은 mobile workspace dev dependency에 고정되어 있으므로 `npm install` 후 별도 전역 설치 질문 없이 실행할 수 있습니다.

다른 사람이 설치할 수 있는 Android 테스트 APK는 Expo 계정 로그인 후 `apps/mobile` 기준으로 `npm run build:android:preview`를 실행해 EAS internal distribution build로 생성합니다. EAS 프로젝트는 `@khsol/bim-photo-sync` / `c67aaba8-2622-4ea4-b17d-bb34a380d088`로 연결되어 있습니다. 최신 Android preview APK는 EAS build `a3a50067-ab9a-48e3-a535-353d95ccaae8`에서 생성했고, GitHub Releases의 `BIMPhotoSync-Android-preview-v0.1.4.apk` 자산으로 배포합니다. 해당 APK의 SHA256은 `54C5ECFF139C909F09A4F44044CBDDD652C1337478F48CC0E30A6CC69AA58C7F`입니다. iOS는 Apple 정책상 TestFlight 또는 Ad Hoc/Enterprise 배포가 필요하므로 Apple Developer 계정과 기기 등록 또는 App Store Connect 설정이 필요합니다. `apps/mobile/app.json`에는 `ITSAppUsesNonExemptEncryption: false`가 설정되어 있으며, iOS preview build는 `npm run build:ios:preview`, Android+iOS 동시 preview build는 `npm run build:preview`, 스토어 제출용 Android AAB는 `npm run build:android:production`, iOS 스토어 빌드는 `npm run build:ios:production`을 사용합니다.
...
- 3D Model이 Floor Plan별 sectioned OBJ를 업로드하고 `%APPDATA%\BimPhotoSync\validation.log`에 각 Floor Plan의 section box source(view-scoped Room, same-level document fallback, crop-box fallback)와 `Synced N Floor Plan section 3D models.` 결과를 남깁니다.
...
- [Autodesk Construction Cloud BIM Coordination](https://construction.autodesk.com/workflows/bim-coordination-collaboration)
- [Autodesk Revit Cloud Worksharing](https://help.autodesk.com/cloudhelp/2024/ENU/Revit-Cloud/files/GUID-9C74D6BC-2FBD-40B0-A27B-CDC766D35675.htm)
- [Autodesk Model Coordination](https://help.autodesk.com/cloudhelp/ENU/BIM360D-Model-Coordination/files/GUID-38CC3A1C-92FF-4682-847F-9CFAFCC4CCCE.html)
