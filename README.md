...
- My Page는 로그인 사용자 이름, 역할, 회사명, Revit Add-in 사용 흐름, 계정 정보를 표시합니다.
...
2. ZIP 압축을 풉니다.
3. 압축 해제 폴더에서 PowerShell을 열고 `powershell -ExecutionPolicy Bypass -File .\install.ps1`을 실행합니다.
4. Revit 2025를 다시 시작합니다.
5. Revit 상단의 `BIM Photo Sync` 탭이 보이면 설치가 완료된 상태입니다.

삭제는 같은 패키지의 `uninstall.ps1`을 실행합니다.
